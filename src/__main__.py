...
from api.service.controller.station_controller import StationController
...
def run(host: str = '127.0.0.1', port: int = 12080):
    ...
    if GlobalData().mode == 'release':
        test_app.add_api('api/swagger/swagger.yaml')
        server = pywsgi.WSGIServer((host, port), test_app)
        server.serve_forever()
    else:
        test_app.add_api('api/swagger/swagger.yaml', resolver_error=501)
